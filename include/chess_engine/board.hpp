...
private:
    ...
    bool white_castle_kingside;   // Can White castle kingside (O-O)? Requires: king and rook haven't moved, no pieces between them
    bool white_castle_queenside;  // Can White castle queenside (O-O-O)? Same conditions as kingside
    ...
    Square en_passant_square;     // Square where en passant capture is possible. Occurs when pawn moves 2 squares forward
    int halfmove_clock;           // Number of half-moves since last capture or pawn move (for 50-move rule)
