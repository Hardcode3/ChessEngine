...
private:
  ...
  void
  generate_pawn_moves ( std::vector<Move> &moves, Square square ) const;
  void
  generate_knight_moves ( std::vector<Move> &moves, Square square ) const;
  void
  generate_bishop_moves ( std::vector<Move> &moves, Square square ) const;
  void
  generate_rook_moves ( std::vector<Move> &moves, Square square ) const;
  void
  generate_queen_moves ( std::vector<Move> &moves, Square square ) const;
  void
  generate_king_moves ( std::vector<Move> &moves, Square square ) const;

  bool
  is_square_attacked ( Square square, Color attacker ) const;
  bool
  is_in_check ( Color color ) const;
  bool
...
public:
  ...
  void
  load_fen ( const std::string &fen );
  std::string
  get_fen () const;

  std::vector<Move>
  generate_legal_moves () const;

  void
  make_move ( Move &move );
  void
  unmake_move ( const Move &move );
  ...
  Move
  uci_to_move ( const std::string &uci_move ) const;
  std::string
  move_to_uci ( const Move &move ) const;

  Piece
  get_piece ( Square square ) const
  {
    return pieces[square.rank][square.file];
  }
  Color
  get_color ( Square square ) const
  {
    return colors[square.rank][square.file];
  }
  Color
  get_side_to_move () const
  {
    return side_to_move;
  }
  bool
