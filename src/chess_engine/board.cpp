...
Board::Board()
    : pieces{{{Piece::EMPTY}}}, // Initializes all elements to Piece::EMPTY
      ...
      side_to_move(Color::WHITE), white_castle_kingside(true),
      white_castle_queenside(true), black_castle_kingside(true),
      black_castle_queenside(true), en_passant_square(0, 0), halfmove_clock(0),
      fullmove_number(1) {}
...
std::vector<Move> Board::generate_legal_moves() const {
  ...
  for (int rank = 0; rank < 8; ++rank) {
    for (int file = 0; file < 8; ++file) {
      ...
      if (this->get_piece(square) != Piece::EMPTY &&
          this->get_color(square) == side_to_move) {
  ...
  moves.erase(std::remove_if(moves.begin(), moves.end(),
                             [this](const Move &move) {
                               return !this->is_move_legal(move);
                             }),
              moves.end());
...
void Board::make_move(Move &move) {
  ...
  if (move.piece == Piece::PAWN) {
    ...
    if (move.to == this->en_passant_square) {
      this->pieces[move.to.rank - (this->side_to_move == Color::WHITE ? 1 : -1)]
                  [move.to.file] = Piece::EMPTY;
    }
    // Handle promotion
  ...
  // Update game state
  this->side_to_move =
      (this->side_to_move == Color::WHITE) ? Color::BLACK : Color::WHITE;
...
void Board::generate_pawn_moves(std::vector<Move> &moves, Square square) const {
  ...
  const int direction =
      (pawn_color == Color::WHITE) ? 1 : -1; // White moves up, black moves down
  const int starting_rank = (pawn_color == Color::WHITE)
                                ? 1
                                : 6; // White starts at rank 1, black at rank 6
  const int promotion_rank =
      (pawn_color == Color::WHITE)
          ? 7
          : 0; // White promotes at rank 7, black at rank 0

  // Forward moves
  const Square one_forward(square.file, square.rank + direction);
  if (one_forward.rank >= 0 && one_forward.rank < 8 &&
      this->get_piece(one_forward) == Piece::EMPTY) {
    // Check for promotion
    if (one_forward.rank == promotion_rank) {
      moves.emplace_back(square, one_forward, Piece::PAWN, Piece::EMPTY, true,
                         Piece::QUEEN);
      moves.emplace_back(square, one_forward, Piece::PAWN, Piece::EMPTY, true,
                         Piece::ROOK);
      moves.emplace_back(square, one_forward, Piece::PAWN, Piece::EMPTY, true,
    ...
    if (target_file >= 0 && target_file < 8) {
      ...
      if (capture_square.rank >= 0 && capture_square.rank < 8) {
        ...
        if (target_piece != Piece::EMPTY &&
            this->get_color(capture_square) != pawn_color) {
          if (capture_square.rank == promotion_rank) {
            moves.emplace_back(square, capture_square, Piece::PAWN,
                               target_piece, true, Piece::QUEEN);
            moves.emplace_back(square, capture_square, Piece::PAWN,
                               target_piece, true, Piece::ROOK);
            moves.emplace_back(square, capture_square, Piece::PAWN,
                               target_piece, true, Piece::BISHOP);
            moves.emplace_back(square, capture_square, Piece::PAWN,
                               target_piece, true, Piece::KNIGHT);
          } else {
            moves.emplace_back(square, capture_square, Piece::PAWN,
                               target_piece);
          }
...
void Board::generate_knight_moves(std::vector<Move> &moves,
  ...
  const int directions[8][2] = {{2, 1}, {2, -1}, {-2, 1}, {-2, -1},
                                {1, 2}, {1, -2}, {-1, 2}, {-1, -2}};
